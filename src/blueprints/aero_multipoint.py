...
# 1. MESH GENERATION
# =============================================================================
# Rectangular meshes return only mesh; CRM meshes return (mesh, twist_cp).
# Always use the guarded unpack below so changing wing_type does not crash.
...
if isinstance(_mesh_result, tuple):
    ...
    mesh = _mesh_result
    twist_cp = np.zeros(mesh_dict.get("num_twist_cp", 5))
...
surf_dict = {
    ...
    "S_ref_type": "wetted",
    "mesh": mesh,
    # Required for twist optimization; for rect wings this starts at zero twist.
    "twist_cp": twist_cp,
    ...
    # "chord_cp": np.ones(len(twist_cp)),   # Chord scaling CPs (1.0 = no scaling)
    # "xshear_cp": np.zeros(len(twist_cp)), # x-shear CPs [m] — generalized sweep
    # "zshear_cp": np.zeros(len(twist_cp)), # z-shear CPs [m] — generalized dihedral
...
mach_numbers = np.array([0.78, 0.84])
rho_values = np.array([0.38, 0.32])
velocity_values = mach_numbers * 295.0
re_values = rho_values * velocity_values / 1.81e-5
cl_targets = np.array([0.45, 0.50])
cd_weights = np.array([0.50, 0.50])
n_points = len(mach_numbers)
assert len(rho_values) == len(cl_targets) == len(cd_weights) == n_points
cd_weights = cd_weights / np.sum(cd_weights)

indep_var_comp.add_output("v", val=velocity_values, units="m/s")
indep_var_comp.add_output(
    "alpha", val=np.ones(n_points) * 4.0, units="deg"
)  # shape=(n_points,)
indep_var_comp.add_output("Mach_number", val=mach_numbers)
indep_var_comp.add_output("re", val=re_values, units="1/m")
indep_var_comp.add_output("rho", val=rho_values, units="kg/m**3")
...
prob.model.add_subsystem(
    "weighted_CD",
    om.ExecComp(
        "CD = " + " + ".join(f"{cd_weights[i]:.16g}*CD{i}" for i in range(n_points))
    ),
)
for i in range(n_points):
    prob.model.connect(f"aero_point_{i}.wing_perf.CD", f"weighted_CD.CD{i}")
...
#   "weighted_CD.CD"                  — weighted drag objective from ExecComp

for i in range(n_points):
    prob.model.add_constraint(f"aero_point_{i}.wing_perf.CL", equals=cl_targets[i])

# --- Objective ---
# Use ExecComp for weighted drag; OpenAeroStruct MultiCD has no weights option.
# FULL OBJECTIVE PATH REFERENCE:
#   "weighted_CD.CD"                  — weighted sum of drag
