...
prob.model.add_design_var("wing.taper", lower=0.1, upper=2.0)

prob.model.add_constraint(f"{point_name}.wing_perf.CL", equals=0.5)
prob.model.add_constraint(f"{point_name}.wing_perf.S_ref", equals=400.0)
...
try:
    ...
    Cl_dist = prob.get_val(f"{point_name}.wing_perf.Cl")
    span_locs = np.linspace(-30, 30, len(Cl_dist))
    axes[0].plot(span_locs, Cl_dist, marker='o')
    ...
    axes[1].set_title("Optimized Wing Planform")
